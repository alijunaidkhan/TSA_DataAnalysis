...
from PyQt6.QtCore import Qt, QUrl,pyqtSignal,QDateTime,QThread,QModelIndex,QObject
...
# Add Split Dataset action with an icon to the Model menu
    ...
    def forecast_dialogue_multivariate(self):
        ...
        if self.X_test_multi is None:
         ...
         self.setWindowIcon(QIcon(icon_path))
         custom_color = QColor("#B22222")  # OrangeRed color
         message_text = "Please perform modeling for future forecasting. Navigate to menu Model > Nueral Network > Multivariate RNN."  # Example message text
         window_title = "Modeling Not Done"  # Example window title
         icon_text = "!"  # Example icon text
         CustomMessageBox(custom_color,message_text, window_title, icon_text, self).exec() 
         icon_path = os.path.abspath('images/bulb_icon.png')
...
class SubsetDialog(QDialog):
    ...
    def initUI(self):
        ...
        # Create a widget that will hold the form layout
        self.scroll_widget = QWidget()
        self.formLayout = QFormLayout(self.scroll_widget)
        self.scroll_widget.setLayout(self.formLayout)
    ...
    def getValues(self):
        ...
        for column, (min_input, max_input) in self.inputs.items():
            min_val = min_input.text().strip() or "0"
            max_val = max_input.text().strip() or "0"
            try:
                ...
                custom_message_box = CustomMessageBox(icon_color=QColor('#B22222'),  # The specified color for the error icon
                                                    message_text=f"Please enter valid numeric values for {column}.",
                                                    window_title="Invalid Input",
                                                    icon_text='X',
                                                    parent=self)
                custom_message_box.exec()
                return {}
    ...
    def show_result_message(self, num_subsets, subset_lengths):
        ...
        custom_message_box = CustomMessageBox(icon_color=QColor("#4682B4")  ,  # Specify the color for the information icon (dodger blue color)
                                          message_text=f"Recommendation:{best_subset} Subset is recommended for analysis.",
                                          window_title="Subset Information",
                                          icon_text='i',
                                          parent=self)
        custom_message_box.exec()
    ...
    def save_subsets(self, subsets):
        initial_directory = os.path.dirname(self.parent().file_path)
        filename = os.path.join(initial_directory, f"TSA_{os.path.basename(self.parent().file_path).split('.')[0]}_Subsets.zip")
        file_path, _ = QFileDialog.getSaveFileName(self, "Save as ZIP", filename, "ZIP Files (*.zip)")
        ...
        if file_path:
            ...
            custom_color = QColor("#5cb85c")  # Green color
            message_text = f"Subsets saved to {save_path}"
            window_title = "Successfully Saved Subset"
            icon_text = "✔"
...
# Create a Matplotlib figure
    ...
    def train_model(self):
        ...
        if NN_df.isnull().any():
            warning_message = f"NaN values found in column '{selected_column}'. Please go to Preprocess menu and perform imputation to fill the missing values."
            ...
            return
        params = (timesteps, num_units, dense_activation, epochs, dense_units, selected_column,
        ...
        self.train_thread.signals.training_finished.connect(self.on_training_finished)
        self.train_thread.signals.training_error.connect(self.handle_error)

        self.run_button.setEnabled(False)
        self.train_thread.start()

    def on_training_finished(self, model, train_results, val_results, test_results,test_loss,test_rmse,X_test_scaled,y_train,y_test,y_val):
        self.tab_widget.setCurrentIndex(0)
        self.model = model
        self.y_test=y_test
...
class TrainerSignals(QObject):
    training_finished = pyqtSignal(object, pd.DataFrame, pd.DataFrame, pd.DataFrame, float, float,object,object,object,object)  # Emit model and results
...
class TrainThread(QThread):
    ...
    def run(self):
        try:
            # Unpack parameters
            (timesteps, num_units, dense_activation, epochs, dense_units, selected_column,
            ...
            self.y_train=y_train
            self.y_val=y_val
            model = build_model(input_shape, num_units, dense_activation, dense_units, epochs, X_train_scaled, y_train, X_val_scaled, y_val)
            ...
            self.signals.training_finished.emit(model, train_results, val_results, test_results,test_loss,test_rmse,X_test_scaled,y_train,y_test,y_val)
        except Exception as e:
